...
with dag:
    run_print_hello_world = PythonOperator(
        task_id='run_print_hello_world',
        python_callable=run_print_hello_world
    )
    ...
    run_print_hello_world_again = PythonOperator(
        ...
        op_kwargs={'file_path': '{{ ti.xcom_pull(task_ids="run_write_to_tmp_dir_on_local_machine") }}'}
    )

    run_print_final_hello = PythonOperator(
        task_id='run_print_final_hello',
        python_callable=run_print_final_hello
    )

    run_print_hello_world >> run_write_to_tmp_dir_on_local_machine >> run_print_hello_world_again >> run_print_final_hello
